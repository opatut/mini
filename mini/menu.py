...
class Menu(object):
    ...
    def getTree(self, subtree=""):
        tree = []
        for item in self.items:
            if item.path == subtree:
                item.children = self.getTree(("%s.%s" % (subtree, item.name)) if subtree else item.name)
                tree.append(item)
        tree.sort(key=lambda item: item.index)
        return tree
